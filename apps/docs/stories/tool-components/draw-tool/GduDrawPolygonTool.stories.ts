...
const meta = {
  title: "工具组件/绘制工具/绘制多边形",
  component: GduDrawPolygonTool,
} satisfies Meta<typeof GduDrawPolygonTool>;

export default meta;
type Story = StoryObj<typeof meta>;

export const Primary: Story = {
  name: "GduDrawPolygonTool",
  args: {
    ...defaultDrawPolygonProps,
  },
  render: (args) => ({
...
<!--      />-->
    ...
    setup() {
      ...
      function startDraw() {
        drawPolygon.value.startDraw();
      }

      function finishDraw() {
        drawPolygon.value.finishDraw();
      }

      function cancelDraw() {
        drawPolygon.value.cancelDraw();
      }
