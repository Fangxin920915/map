...
const meta = {
  title: "几何图层/矢量图层/基础几何/面/单面",
  component: GduPolygon,
} satisfies Meta<typeof GduPolygon>;
  ...
  render: (args) => ({
    template: `
      ...
      <gdu-vector-layer v-if="show" :view-id="viewId">
        <template v-for="({geometry},index) in polygons" :key="index">
          <gdu-polygon  v-bind="args" :coordinates="geometry.coordinates"/>
        </template>
      </gdu-vector-layer>
      <gdu-terrain-basemap
          v-if="show"
    ...
    setup() {
      ...
      function createPolygons() {
        polygons.value = turf.randomPolygon(400, {
          bbox: [70, 10, 150, 60],
          num_vertices: 15,
          max_radial_length: 1,
        }).features;
